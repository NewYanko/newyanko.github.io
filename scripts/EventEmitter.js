    ...
    EventEmitter._ = {
      checkMaxListeners: function (emitter, eventName) {
        ...
        if (len > emitter.getMaxListeners()) {
          console.warn(
            "possible EventEmitter memory leak detected. %d %s listeners added. Use emitter.setMaxListeners() to increase limit.",
            len,
            eventName
          );
        }
      ...
      append: function (emitter, eventName, listener, origListener) {
        ...
        (emitter._events[eventName] = emitter._events[eventName] || []).push(
          listener
        );
      ...
      prepend: function (emitter, eventName, listener, origListener) {
        emitter._events = emitter._events || {};
        emitter.emit("newListener", eventName, origListener || listener);
        (emitter._events[eventName] = emitter._events[eventName] || []).splice(
          0,
          0,
          listener
        );
